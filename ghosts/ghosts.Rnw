...
<<fitmodel, warning=FALSE, message=FALSE>>=
...
<<userfuns, warning=FALSE, message=FALSE>>=
...
<<ccalcps, warning=FALSE, message=FALSE>>=
...
So with this detection function, we expect animals to be detected between \Sexpr{round(min(Ens),1)} and \Sexpr{round(max(Ens),1)} times, depending on where their activity centers are.
...
<<pplots, warning=FALSE, message=FALSE>>=
...
<<ccalcover, warning=FALSE, message=FALSE>>=
coveratio = 100*(1-coverage2plus/coverage.)
@

From which we see that you'd expect to detect about \Sexpr{round(coveratio)}\% more animals than animals that were detected twice, if there were no ghosts.
\end{document}
