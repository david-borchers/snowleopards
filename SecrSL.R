...
covariates(traps(all.data.Nemegt))$stdGRIDCODE = scale(covariates(traps(all.data.Nemegt))$GRIDCODE)

plot(NemegtMask1, covariate="GRIDCODE", contour=FALSE, col=terrain.colors(10), legend = FALSE)
...
Nemegt.hhn.DRgd1<-secr.fit(all.data.Nemegt, model=list(D~GRIDCODE, lambda0~1, sigma~1), detectfn="HHN", mask=NemegtMask1)
...
AIC(Nemegt.hhn, Nemegt.hhn.DRgd1, Nemegt.hhn.DRgd1, Nemegt.hhn.Det_Topo,Nemegt.hhn.Det_Topo1, Nemegt.DRgd.Det_Topo)
