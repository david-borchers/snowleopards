...
# Kruger Park leopards simulation code
# ------------------------------------
...
dbad = cbad = 0
for(i in 1:nsim) {
  simpop = sim.popn(D,mask.utm, buffer=0, model2D="IHP")
  simch = sim.capthist(cams,popn=simpop,detectfn="HHN",detectpar=list(labmda0=dpar$lambda0,sigma=dpar$sigma),noccasions=1)
  cDfit = secr.fit(simch, mask=mask.utm,detectfn="HHN", start=list(D=trueD,labmda0=dpar$lambda0,sigma=dpar$sigma),trace=FALSE)
