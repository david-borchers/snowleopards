...
TNNfit.DGridSess.lamSess.SigSess <- secr.fit(TNN_ch, detectfn="HHN", mask=list(TostMask, NoyonMask, NemegtMask),
                                             model=list(D~rmeanGC + rmeanGCdev:session + session, lambda0~session, sigma~session), ncores=ncores)
...
TNN.DGrid.LamTopo.Sig1<-secr.fit(TNN_ch, detectfn="HHN", mask = list(TostMask, NoyonMask, NemegtMask),
                                 model = list(D~rmeanGC + rmeanGCdev: session + session, lambda0~Topo, sigma~1), ncores=ncores)
...
TNN.DGridXsess.LamTopoXsess<-secr.fit(TNN_ch, detectfn="HHN", mask = list(TostMask, NoyonMask, NemegtMask),
                                      model = list(D~rmeanGC + rmeanGCdev: session + session, lambda0~Topo*session, sigma~1), ncores=ncores)
...
TNN.DGridXsess.DetTopoXsess<-secr.fit(TNN_ch, detectfn="HHN", mask = list(TostMask, NoyonMask, NemegtMask),
                                      model = list(D~rmeanGC + rmeanGCdev: session + session, lambda0~Topo*session, sigma~session), ncores=ncores)
...
TNNfit.D1.Lam1s.sig1 <- secr.fit(TNN_ch, detectfn="HHN", mask=list(TostMask, NoyonMask, NemegtMask),
                                 model=list(D~1, lambda0~1, sigma~1), ncores=ncores)
...
TNNfit.DGrid.a0Topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                   model=list(D~stdGC, a0~Topo, sigma~1), ncores=ncores)
#8.1 Do lambda0 instead of a0:  (*** David added ***)
TNNfit.DGrid.lam0Topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                   model=list(D~stdGC, lambda0~Topo, sigma~1), ncores=ncores)
...
TNNfit.DGrid.a0Topo_sess.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                   model=list(D~stdGC, a0~Topo+session, sigma~1), ncores=ncores)
...
TNNfit.DGridxSess.a0Topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                        model=list(D~stdGC*session, a0~Topo, sigma~1), ncores=ncores)
...
TNNfit.DGrid.a0Water.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                   model=list(D~stdGC, a0~Water, sigma~1), ncores=ncores)
...
TNNfit.DGrid.a0Waterxsess.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                    model=list(D~stdGC, a0~Water*session, sigma~1), ncores=ncores)
...
TNNfit.DGrid.a0Water_topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                         model=list(D~stdGC, a0~Water+Topo, sigma~1), ncores=ncores)
...
# 8h Density varies with ruggedness, constant for three session. a0 varies with water at trap site and session
TNNfit.D2Grid.a0Waterxsess_topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                              model=list(D~s(stdGC,k=3), a0~Topo+Water*session, sigma~1), ncores=ncores)
# Refit because variance estiamtion failed:
TNNfit.D2Grid.a0Waterxsess_topo.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                               model=list(D~s(stdGC,k=3), a0~Topo+Water*session, sigma~1), ncores=ncores,
                                               start=TNNfit.D2Grid.a0Waterxsess_topo.sig1)
...
TNNfit.DGrid.a0TopoSess.sigsess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                          model=list(D~stdGC, a0~Topo*session, sigma~session), ncores=ncores)
...
TNNfit.DSess.a0TopoSess.sigSess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                          model=list(D~session, a0~Topo*session, sigma~session), ncores=ncores)
...
TNNfit.D1.a0TopoSess.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                    model=list(D~1, a0~Topo*session, sigma~1), ncores=ncores)
...
TNNfit.DGridSess.a0TopoSess.sig1<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                           model=list(D~rmeanGC + rmeanGCdev: session + session, a0~Topo*session, sigma~1), ncores=ncores)
...
TNNfit.DGridSess.a0TopoSess.sigsess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                              model=list(D~rmeanGC + rmeanGCdev: session + session, a0~Topo*session, sigma~session), ncores=ncores)
...
TNNfit.DGridSess.a0Sess.sigsess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                          model=list(D~rmeanGC + rmeanGCdev: session + session, a0~session, sigma~session), ncores=ncores)
...
TNNfit.DGridXSess.a0Sess.sigsess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                           model=list(D~stdGC*session, a0~session, sigma~session), ncores=ncores)
...
TNNfit.DGrid_Sess.a0TopoXSess.sigsess<-secr.fit(TNN_ch,detectfn="HHN", list(TostMask, NoyonMask, NemegtMask),
                                                model=list(D~stdGC+session, a0~Topo*session, sigma~session), ncores=ncores)
...
save(TNNfit.DGridSess.lamSess.SigSess, TNN.DGrid.LamTopo.Sig1, TNN.DGridXsess.LamTopoXsess, TNN.DGridXsess.DetTopoXsess, TNNfit.D1.LamSess.sig1, 
    ...
    TNNfit.D2Grid.a0Waterxsess_topo.sig1,
    TNNfit.DGrid.lam0Topo.sig1,
    file = "./Analysis4paper/TNNnoNEfits.RData")
...
=======
>>>>>>> da2bd043401f24e7163fa55f52285121ade192c7
